...
impute_median <- function(path) {
  ...
  # We use the chaining operator from dplyr here
  df <- df %>% mutate_all(~ ifelse(is.na(.), median(., na.rm = TRUE), .))

  df[which(df$petal_length >= 5.5), ]

  return(median(df[which(df$petal_length >= 5.5), "sepal_length"]))
}
```
...
top5_words <- function(x) {
  ...
  words <- unlist(strsplit(tolower(x), "\\W+"))

  # Create a frequency table of lowercase words
  word_freq <- table(words)

  # Sort the words by frequency in descending order
  sorted_words <- names(sort(word_freq, decreasing = TRUE))

  # Extract the top 5 most occurring lowercase words
  top_words <- head(sorted_words, 5L)
...
workdays_until_xmas <- function(day) {
  ...
  xmas <- as.Date("25-12-2023", format = "%d-%m-%Y") # Xmas date

  # Convert input day to Date format
  input_date <- as.Date(paste(day, "-2023", sep = ""), format = "%d-%m-%Y")

  # Define a function to check if a given date is a weekend (Saturday or Sunday)
  is_weekend <- function(date) {
    # We use the wday function from lubridate here
    wday(date, week_start = 1) %in% c(6, 7)
  }

  # Calculate the number of workdays between input_date and xmas
  workdays <- 0L
  current_date <- input_date
  while (current_date < xmas) {
    if (!is_weekend(current_date)) {
      workdays <- workdays + 1L
    }
